"""
Expanded-MIMIC adapter for IMM-TFS (v3 fixed/nested protocol capable).
...
5. v3/fixed mode: train/val/test IDs are persisted once, while each Train_N
   fits its OWN feature-wise scaler from Train_N HISTORY only;
...
def _load_fixed_protocol(dataset_path: str):
    ...
    if protocol.get("version") != "3-per-n-normalization":
        raise RuntimeError(
            "Fixed MIMIC protocol is from an older v3 variant. "
...
def parse_datasets(args, show_summary=True):
    """Drop-in parser with optional v3 fixed/nested MIMIC protocol."""
    ...
    if fixed_mode:
        protocol, protocol_path = _load_fixed_protocol(dataset_path)
        ...
        # Formal scaling protocol: every Train_N fits its OWN scaler from
        # exactly those N training subjects and HISTORY [0, history) only.
        # This keeps N=200 from using statistics from the rest of the 12,145
        # subject train pool. Uni/Multi at the same N deterministically obtain
        # identical mean/std because they use the exact same train IDs.
        ds.fit_and_apply_train_history_normalization(train_idx)
        ds.normalization_source = (
            f"fixed protocol Train_N={fixed_train_n}; history observations only"
        )
        ...
        print("normalization file     : none (fitted per Train_N at run time)")
        ...
        print(f"normalization          : Train_N={fixed_train_n} HISTORY-only z-score")
    ...
    if fixed_mode:
        out["fixed_protocol"] = {
            ...
            "normalization_scope": f"Train_N={fixed_train_n} history only",
        }
    return out
