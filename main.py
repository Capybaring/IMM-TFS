...
def trainable(
    ...
    # GPINet with pre-computed embeddings owns its text path: reports receive
    # independent variable-time gates, are diffused over the historical grid
    # by time distance, and are fused before MTGNN. Other models (and raw-text
    # GPINet runs) keep the benchmark's external FusionModel.
    ...
    if native_text_fusion:
        logger.info(
            "Text route: GPINet independent variable-time gate + Gaussian diffusion"
        )
    ...
    for itr in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            if (
                native_text_fusion
                and "text_variable_gate_mean" in val_res
            ):
                logger.info(
                    "Val - Variable gate mean/max/spread, time weight, "
                    "context RMS, update abs: {:.5f}, {:.5f}, {:.5f}, "
                    "{:.5f}, {:.5f}, {:.5f}".format(
                        val_res.get(
                            "gpinet_text_variable_gate_mean",
                            float("nan"),
                        ),
                        val_res.get(
                            "gpinet_text_variable_gate_max",
                            float("nan"),
                        ),
                        val_res.get(
                            "gpinet_text_variable_gate_spread",
                            float("nan"),
                        ),
                        val_res.get(
